...
@interface DBPhotoViewController () <DBRestClientDelegate>
...
- (void)loadRandomPhoto {
    if ([photoPaths count] == 0) {
        ...
        NSString *msg = nil;
        if ([DBSession sharedSession].root == kDBRootDropbox) {
            msg = @"Put .jpg photos in your Photos folder to use Biergarten!";
        } else {
            msg = @"Put .jpg photos in your app's App folder to use Biergarten!";
        }
...
- (NSString*)photoPath {
    return [NSTemporaryDirectory() stringByAppendingPathComponent:@"i1.jpg"];
}
